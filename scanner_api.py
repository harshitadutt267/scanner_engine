...
async def scan_code(file: UploadFile = File(...)):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            if lang == "python":
                from python_v2.python_scanner import run_scan as python_run_scan
                result = python_run_scan(file_path)
                return JSONResponse(content={"language": lang, "result": result})
            elif lang == "terraform":
                from terraform_v2.scanner_project import run_terraform_scan
                result = run_terraform_scan([file_path])
                return JSONResponse(content={"language": lang, "result": result.get("findings", [])})
            else:
...
# Improved /scan-folder endpoint: supports both .py and .tf files
...
async def scan_folder(zip_file: UploadFile = File(...)):
    ...
    try:
        ...
        # Use /code/scan_code for input extraction, /code/scan_result for output
        input_dir = "/code/scan_code"
        output_dir = "/code/scan_result"

        # Clean input_dir before extracting new code
        import shutil
        if os.path.exists(input_dir):
            shutil.rmtree(input_dir)
        os.makedirs(input_dir, exist_ok=True)

        zip_path = os.path.join(input_dir, zip_file.filename)
        with open(zip_path, "wb") as f:
            f.write(await zip_file.read())
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
        ...
        # Store results to /code/scan_result (host-mountable directory)
        project_name = os.path.splitext(zip_file.filename)[0]
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        results_dir = os.path.join(output_dir, project_name, timestamp)

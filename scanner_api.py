...
async def scan_folder(zip_file: UploadFile = File(...)):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for root, _, files in os.walk(tmpdir):
            for fname in files:
                ...
                try:
                    ...
                    logger.warning(f"Could not detect language for {fname}: {str(e)}")
                    continue
        ...
        for lang, files in files_by_language.items():
            try:
                # Get appropriate scanner for the language
                run_scan = get_scanner(lang)
                
                # For Terraform, use project-level scanning
                if lang == "terraform":
                    from terraform_v2.scanner_project import run_terraform_scan
                    lang_results = run_terraform_scan(files)
                else:
                    # For other languages, scan individual files
                    lang_results = []
